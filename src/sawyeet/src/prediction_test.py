...
def stateCallback(coords):
    ...
    if rospy.Time.now().to_sec() - prev_time > 0.3 or not IS_INITIALIZED:
        ...
        pred_state[2] = prev_state[2] + prev_state[5] * dT - ETA * prev_state[5] * dT**2 / 2.
        pred_state[3] = prev_state[3]
        pred_state[4] = prev_state[4] - G * dT
        pred_state[5] = prev_state[5] - ETA * prev_state[5] * dT
...
def estimation():
    ...
    if rospy.Time.now().to_sec() - prev_time > 0.3 and curr_state[5] != 0:
        pred_time = (curr_state[5] +  (curr_state[5] + 2 * ETA * curr_state[5] * (curr_state[2] + DIST_Z))**0.5)/(ETA * curr_state[5])
        print(pred_time)
